...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a Ventual initiative\n"
            "  vote-alias         - Vote on a Ventual initiative\n"
            "  vote               - Vote on a Ventual initiative/budget\n"
            "  getvotes           - Show current vnode budgets\n"
            "  getinfo            - Show current vnode budgets\n"
...
Value preparebudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"ventual-address\" monthy-payment\n"
            ...
            "5. \"ventual-address\":   (string, required) Ventual address to send payments to\n"
            "6. monthly-payment:  (numeric, required) Monthly payment amount\n"

            "\nResult:\n"
            "\"xxxx\"       (string) proposal fee hash (if successful) or error message (if failed)\n"
            "\nExamples:\n" +
            HelpExampleCli("preparebudget", "\"test-proposal\" \"https://forum.ventual.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("preparebudget", "\"test-proposal\" \"https://forum.ventual.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));
...
Value submitbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"ventual-address\" monthy-payment \"fee-tx\"\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("submitbudget", "\"test-proposal\" \"https://forum.ventual.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500") +
            HelpExampleRpc("submitbudget", "\"test-proposal\" \"https://forum.ventual.org/t/test-proposal\" 2 820800 \"D9oc6C3dttUbv8zd7zGNq1qKBGf4ZQ1XEE\" 500"));
    ...
    if (!vnodeSync.IsBlockchainSynced()) {
        throw runtime_error("Must wait for client to sync with vnode network. Try again in a minute or so.");
    }
    ...
    budget.mapSeenVnodeBudgetProposals.insert(make_pair(budgetProposalBroadcast.GetHash(), budgetProposalBroadcast));
...
Value mnbudgetvote(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() == 3 && (strCommand != "local" && strCommand != "many")) || (params.size() == 4 && strCommand != "alias") ||
        ...
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mode\"      (string, required) The voting mode. 'local' for voting directly from a vnode, 'many' for voting with a MN controller and casting the same vote for each MN, 'alias' for voting with a MN controller and casting a vote for a single MN\n"
    ...
    if (strCommand == "local") {
        ...
        while (true) {
            if (!obfuScationSigner.SetKey(strVNodePrivKey, errorMessage, keyVnode, pubKeyVnode)) {
                ...
                statusObj.push_back(Pair("error", "Vnode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                ...
                statusObj.push_back(Pair("node", "local"));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Failure to find vnode in list : " + activeVnode.vin.ToString()));
                resultsObj.push_back(statusObj);
                break;
            }

            CBudgetVote vote(activeVnode.vin, hash, nVote);
            ...
            if (budget.UpdateProposal(vote, NULL, strError)) {
                success++;
                budget.mapSeenVnodeBudgetVotes.insert(make_pair(vote.GetHash(), vote));
                vote.Relay();
                statusObj.push_back(Pair("node", "local"));
    ...
    if (strCommand == "many") {
        BOOST_FOREACH (CVnodeConfig::CVnodeEntry mne, vnodeConfig.getEntries()) {
            ...
            if (!obfuScationSigner.SetKey(mne.getPrivKey(), errorMessage, keyVnode, pubKeyVnode)) {
                failed++;
            ...
            CVnode* pmn = mnodeman.Find(pubKeyVnode);
            if (pmn == NULL) {
                failed++;
                statusObj.push_back(Pair("node", mne.getAlias()));
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("error", "Can't find vnode by pubkey"));
                resultsObj.push_back(statusObj);
                continue;
            }
...
Value getbudgetvotes(const Array& params, bool fHelp)
{
    if (params.size() != 1)
        throw runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"mnId\": \"xxxx\",        (string) Hash of the vnode's collateral transaction\n"
...
Value getbudgetprojection(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) Ventual address of payment\n"
...
Value mnbudgetrawvote(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "mnbudgetrawvote \"vnode-tx-hash\" vnode-tx-index \"proposal-hash\" yes|no time \"vote-sig\"\n"
            "\nCompile and relay a proposal vote with provided external signature instead of signing vote internally\n"

            "\nArguments:\n"
            "1. \"vnode-tx-hash\"  (string, required) Transaction hash for the vnode\n"
            "2. vnode-tx-index   (numeric, required) Output index for the vnode\n"
    ...
    if (pmn == NULL) {
        return "Failure to find vnode in list : " + vin.ToString();
    }
...
Value mnfinalbudget(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "vote-many") {
        ...
        BOOST_FOREACH (CVnodeConfig::CVnodeEntry mne, vnodeConfig.getEntries()) {
            ...
            if (!obfuScationSigner.SetKey(mne.getPrivKey(), errorMessage, keyVnode, pubKeyVnode)) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Vnode signing error, could not set key correctly: " + errorMessage));
            ...
            if (pmn == NULL) {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Can't find vnode by pubkey"));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
    ...
    if (strCommand == "vote") {
        ...
        CVnode* pmn = mnodeman.Find(activeVnode.vin);
        if (pmn == NULL) {
            return "Failure to find vnode in list : " + activeVnode.vin.ToString();
        }

        CFinalizedBudgetVote vote(activeVnode.vin, hash);
        if (!vote.Sign(keyVnode, pubKeyVnode)) {
