...
void MultiSendDialog::updateCheckBoxes()
{
    ...
    ui->multiSendVnodeCheckBox->setChecked(pwalletMain->fMultiSendVnodeReward);
}
...
void MultiSendDialog::on_viewButton_clicked()
{
    ...
    if (pwalletMain->isMultiSendEnabled()) {
        if (pwalletMain->fMultiSendStake)
            ...
            strMultiSendPrint += "MultiSend Active for Vnode Rewards\n";
    } else
...
void MultiSendDialog::on_activateButton_clicked()
{
    ...
    if (pwalletMain->vMultiSend.size() < 1)
    ...
    else if (!(ui->multiSendStakeCheckBox->isChecked() || ui->multiSendVnodeCheckBox->isChecked())) {
        strRet = "Need to select to send on stake and/or vnode rewards\n";
        ...
        pwalletMain->fMultiSendVnodeReward = ui->multiSendVnodeCheckBox->isChecked();

        CWalletDB walletdb(pwalletMain->strWalletFile);
        if (!walletdb.WriteMSettings(pwalletMain->fMultiSendStake, pwalletMain->fMultiSendVnodeReward, pwalletMain->nLastMultiSendHeight))
