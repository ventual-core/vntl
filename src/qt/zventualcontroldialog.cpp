...
#include "ui_zventualcontroldialog.h"
...
std::list<std::string> ZVentualControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVentualControlDialog::listMints;

ZVentualControlDialog::ZVentualControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVentualControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVentualControlDialog::~ZVentualControlDialog()
{
    delete ui;
}

void ZVentualControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVentualControlDialog::updateList()
{
...
void ZVentualControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVentualControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVentual_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVentualControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVentualControlDialog::GetSelectedMints()
{
...
void ZVentualControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
