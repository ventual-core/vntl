...
#include "zventualcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zVNTL ought to be enough for anybody." - Bill Gates, 2017
    ui->zVNTLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVNTLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzVNTL->setEnabled(false);
        ui->pushButtonMintzVNTL->setToolTip(tr("zVNTL is currently disabled due to maintenance."));

        ui->pushButtonSpendzVNTL->setEnabled(false);
        ui->pushButtonSpendzVNTL->setToolTip(tr("zVNTL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVNTLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzVNTL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVNTL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVNTL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVNTL in ") +
...
void PrivacyDialog::on_pushButtonSpendzVNTL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVNTL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zVNTL
        sendzVNTL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVNTL
    sendzVNTL();
}

void PrivacyDialog::on_pushButtonZVentualControl_clicked()
{
    ZVentualControlDialog* zVNTLControl = new ZVentualControlDialog(this);
    zVNTLControl->setModel(walletModel);
    zVNTLControl->exec();
}

void PrivacyDialog::setZVentualControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVNTLSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVNTL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Ventual Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVNTLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVNTL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VENTUAL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVNTL</b>";
    ...
    // use mints from zVNTL selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVentualControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVentualControlDialog::GetSelectedMints();
    }
    ...
    // Clear zventual selector in case it was used
    ZVentualControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVNTL Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Ventual, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVNTL Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVNTL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVNTL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVNTL "));
    ui->labelzVNTLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzVNTLSyncStatus->setVisible(fShow);
}
