...
/* *** BEGIN OBFUSCATION MAGIC - VENTUAL **********
...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        if (pfrom->nVersion < ActiveProtocol()) {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), VNODE_REJECTED, errorID);
        ...
        if (!IsCompatibleWithSession(nDenom, txCollateral, errorID)) {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), VNODE_ACCEPTED, errorID);
            return;
        }
        ...
        if (dsq.ready) {
            ...
            if ((CNetAddr)pSubmittedToVnode->addr != (CNetAddr)addr) {
                LogPrintf("dsq - message doesn't match current Vnode - %s != %s\n", pSubmittedToVnode->addr.ToString(), addr.ToString());
                return;
            }
            ...
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("obfuscation", "dsq -- Vnode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
        ...
        if (AddEntry(in, nAmount, txCollateral, out, errorID)) {
            ...
            RelayStatus(sessionID, GetState(), GetEntriesCount(), VNODE_RESET);
        } else {
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), VNODE_REJECTED, errorID);
        }
        ...
        if ((CNetAddr)pSubmittedToVnode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dssu - message doesn't match current Vnode - %s != %s\n", pSubmittedToVnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (success) {
            ...
            RelayStatus(obfuScationPool.sessionID, obfuScationPool.GetState(), obfuScationPool.GetEntriesCount(), VNODE_RESET);
        }
        ...
        if (!pSubmittedToVnode) return;
        if ((CNetAddr)pSubmittedToVnode->addr != (CNetAddr)pfrom->addr) {
            //LogPrintf("dsc - message doesn't match current Vnode - %s != %s\n", pSubmittedToVnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CObfuscationPool::GetStatus()
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing || !vnodeSync.IsBlockchainSynced()) {
    ...
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            ...
            if (showingObfuScationMessage % 70 <= 40)
                return strprintf(_("Submitted following entries to vnode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                ...
                suffix = "...";
            return strprintf(_("Submitted to vnode, waiting for more entries ( %u / %d ) %s"), entriesCount, GetMaxPoolTransactions(), suffix);
        }
    ...
    case POOL_STATUS_QUEUE:
        if (showingObfuScationMessage % 70 <= 30)
            ...
            suffix = "...";
        return strprintf(_("Submitted to vnode, waiting in queue %s"), suffix);
        ;
    default:
...
//
// Check the Obfuscation progress and send client updates if a Vnode
//
void CObfuscationPool::Check()
{
    if (fVNode) LogPrint("obfuscation", "CObfuscationPool::Check() - entries count %lu\n", entries.size());
    ...
    if (fVNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        ...
        UnlockCoins();
        SetNull();
        if (fVNode) RelayStatus(sessionID, GetState(), GetEntriesCount(), VNODE_RESET);
    }
}
...
void CObfuscationPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        if (!obfuScationSigner.SetKey(strVNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CObfuscationPool::Check() - ERROR: Invalid Vnodeprivkey: '%s'\n", strError);
            return;
        }
...
//
// How does this work? Messages to Vnodes come in via "dsi", these require a valid collateral
    ...
    if (fVNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Ventual and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 VENTUAL per transaction on average.
            */
            if (r <= 10) {
...
{
    if (!fEnableZeromint && !fVNode) return;
    ...
    int addLagTime = 0;
    if (!fVNode) addLagTime = 10000; //if we're the client, give the server a few extra seconds before resetting.
...
//
// Execute a Obfuscation denomination via a Vnode.
// This is only ran from clients
//
void CObfuscationPool::SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount)
{
    if (fVNode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - Obfuscation from a Vnode is not supported currently.\n");
        return;
    }
    ...
    if (!sessionFoundVnode) {
        LogPrintf("CObfuscationPool::SendObfuscationDenominate() - No Vnode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
...
// Incoming message from Vnode updating the progress of Obfuscation
...
bool CObfuscationPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    ...
    if (errorID != MSG_NOERR) strAutoDenomResult = _("Vnode:") + " " + GetMessageByID(errorID);
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            ...
            sessionFoundVnode = true;
            //wait for other users. Vnode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundVnode) {
            LogPrintf("CObfuscationPool::StatusUpdate - entry not accepted by Vnode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
            DoAutomaticDenominating(); //try another Vnode
        }
        if (sessionFoundVnode) return true;
    }
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (mnodeman.size() == 0) {
        LogPrint("obfuscation", "CObfuscationPool::DoAutomaticDenominating - No Vnodes detected\n");
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeVentualAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!sessionFoundVnode) {
        ...
        //if we've used 90% of the Vnode list then drop all the oldest first
        int nThreshold = (int)(mnodeman.CountEnabled(ActiveProtocol()) * 0.9);
        LogPrint("obfuscation", "Checking vecVnodesUsed size %d threshold %d\n", (int)vecVnodesUsed.size(), nThreshold);
        while ((int)vecVnodesUsed.size() > nThreshold) {
            vecVnodesUsed.erase(vecVnodesUsed.begin());
            LogPrint("obfuscation", "  vecVnodesUsed size %d threshold %d\n", (int)vecVnodesUsed.size(), nThreshold);
        }
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                bool fUsed = false;
                //don't reuse Vnodes
                BOOST_FOREACH (CTxIn usedVin, vecVnodesUsed) {
                    if (dsq.vin == usedVin) {
                        fUsed = true;
                        break;
                    }
                ...
                CVnode* pmn = mnodeman.Find(dsq.vin);
                if (pmn == NULL) {
                    LogPrintf("DoAutomaticDenominating --- dsq vin %s is not in vnode list!", dsq.vin.ToString());
                    continue;
                }

                LogPrintf("DoAutomaticDenominating --- attempt to connect to vnode from queue %s\n", pmn->addr.ToString());
                lastTimeChanged = GetTimeMillis();
        ...
        while (i < 10) {
            CVnode* pmn = mnodeman.FindRandomNotInVec(vecVnodesUsed, ActiveProtocol());
            if (pmn == NULL) {
                LogPrintf("DoAutomaticDenominating --- Can't find random vnode!\n");
            ...
            lastTimeChanged = GetTimeMillis();
            LogPrintf("DoAutomaticDenominating --- attempt %d connection to Vnode %s\n", i, pmn->addr.ToString());
            ...
            if (pnode != NULL) {
                ...
                vecVnodesUsed.push_back(pmn->vin); // postpone MN we wasn't able to connect to
                i++;
                continue;
            }
        }
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100VENTUAL+1 ( bit on if present )
    // bit 1 - 10VENTUAL+1
    // bit 2 - 1VENTUAL+1
    // bit 3 - .1VENTUAL+1
    // bit 3 - non-denom
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
            if (out.nValue == 1000 * COIN ||
                out.nValue == 1250 * COIN ||
                out.nValue == 1500 * COIN ||
                out.nValue == 1750 * COIN ||
                out.nValue == 2000 * COIN ||
                out.nValue == 2250 * COIN ||
                out.nValue == 2500 * COIN ||
                out.nValue == 2750 * COIN ||
                out.nValue == 3000 * COIN ) {
...
bool CObfuscationQueue::Sign()
{
    ...
    if (!obfuScationSigner.SetKey(strVNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CObfuscationQueue():Relay - ERROR: Invalid Vnodeprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CObfuscationQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!obfuScationSigner.VerifyMessage(pmn->pubKeyVnode, vchSig, strMessage, errorMessage)) {
            return error("CObfuscationQueue::CheckSignature() - Got bad Vnode address signature %s \n", vin.ToString().c_str());
        }
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("ventual-obfuscation");
    ...
    while (true) {
        ...
        if (vnodeSync.IsBlockchainSynced()) {
            ...
            if (c % VNODE_PING_SECONDS == 1) activeVnode.ManageStatus();

            if (c % 60 == 0) {
                mnodeman.CheckAndRemove();
                mnodeman.ProcessVnodeConnections();
                vnodePayments.CleanPaymentList();
                CleanTransactionLocksList();
            }

            //if(c % VNODES_DUMP_SECONDS == 0) DumpVnodes();
