...
//step 2.) Top SWIFTTX_SIGNATURES_TOTAL vnodes, open connect to top 1 vnode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 vnode, waits for SWIFTTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
        ...
        if (ProcessConsensusVote(pfrom, ctx)) {
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                if (!mapUnknownVotes.count(ctx.vinVnode.prevout.hash)) {
                    mapUnknownVotes[ctx.vinVnode.prevout.hash] = GetTime() + (60 * 10);
                }

                if (mapUnknownVotes[ctx.vinVnode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinVnode.prevout.hash] - GetAverageVoteTime() > 60 * 10) {
                    LogPrintf("ProcessMessageSwiftTX::ix - vnode is spamming transaction votes: %s %s\n",
                        ctx.vinVnode.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which vnodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetVnodeRank(activeVnode.vin, nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    if (n == -1) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Unknown Vnode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::DoConsensusVote - Vnode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
...
{
    int n = mnodeman.GetVnodeRank(ctx.vinVnode, ctx.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

    CVnode* pmn = mnodeman.Find(ctx.vinVnode);
    if (pmn != NULL)
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Vnode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Unknown Vnode\n");
        mnodeman.AskForMN(pnode, ctx.vinVnode);
        return false;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint("swiftx", "SwiftX::ProcessConsensusVote - Vnode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("SwiftX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced vnode
        mnodeman.AskForMN(pnode, ctx.vinVnode);
        return false;
    }
...
{
    return vinVnode.prevout.hash + vinVnode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("SwiftX::CConsensusVote::SignatureValid() - Unknown Vnode\n");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pmn->pubKeyVnode, vchVNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strVNodePrivKey.c_str());

    if (!obfuScationSigner.SetKey(strVNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid vnodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, vchVNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pubkey2, vchVNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetVnodeRank(vote.vinVnode, vote.nBlockHeight, MIN_SWIFTTX_PROTO_VERSION);

        if (n == -1) {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Vnode\n");
            return false;
        }

        if (n > SWIFTTX_SIGNATURES_TOTAL) {
            LogPrintf("CTransactionLock::SignaturesValid() - Vnode not in the top %s\n", SWIFTTX_SIGNATURES_TOTAL);
            return false;
        }
