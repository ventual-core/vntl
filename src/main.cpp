...
#if defined(NDEBUG)
#error "Ventual cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zVNTL spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zVNTL spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zVNTL spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetVnodePayment(int nHeight, int64_t blockValue, int nVnodeCount)
{
    ...
    if (!tx.IsCoinBase() && !tx.IsZerocoinSpend()) {
        ...
        if (!tx.IsCoinStake()) {
            if (nValueIn < tx.GetValueOut())
                return state.DoS(0, error("CheckInputs() : %s value in (%s) < value out (%s)",
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Ventual
...
{
    RenameThread("ventual-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZVENTUALMinted()
{
...
void RecalculateZVENTUALSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zVNTL supply
...
bool RecalculateVENTUALSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zVNTL with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zVNTLSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    //PoW phase redisventuald fees to miner. PoS stage destroys fees.
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert VENTUAL to zVNTL
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u  mna=%m\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble()) / log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
        Checkpoints::GuessVerificationProgress(chainActive.Tip()), (unsigned int)pcoinsTip->GetCacheSize(), mnodeman.CountEnabled());
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // vnode payments / budgets and zerocoin check
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(0, error("CheckBlock() : Couldn't find vnode/budget payment"),
                        REJECT_INVALID, "bad-cb-payee");
            }
        } else {
            if (fDebug)
                LogPrintf("CheckBlock(): Vnode payment check skipped on sync - skipping IsBlockPayeeValid()\n");
        }
    }
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zVNTL spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zVNTL serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zVNTL mints and %d zVNTL spends\n", __func__, nMints, nSpends);
    ...
    if (!fLiteMode) {
        if (vnodeSync.RequestedVnodeAssets > VNODE_SYNC_LIST) {
            obfuScationPool.NewBlock();
            vnodePayments.ProcessBlock(GetHeight() + 10);
            budget.NewBlock();
        }
    }
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_VNODE_WINNER:
        if (vnodePayments.mapVnodePayeeVotes.count(inv.hash)) {
            vnodeSync.AddedVnodeWinner(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_VOTE:
        if (budget.mapSeenVnodeBudgetVotes.count(inv.hash)) {
            vnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_PROPOSAL:
        if (budget.mapSeenVnodeBudgetProposals.count(inv.hash)) {
            vnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    ...
    case MSG_VNODE_ANNOUNCE:
        if (mnodeman.mapSeenVnodeBroadcast.count(inv.hash)) {
            vnodeSync.AddedVnodeList(inv.hash);
            return true;
        }
        return false;
    case MSG_VNODE_PING:
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_VNODE_WINNER) {
                    if (vnodePayments.mapVnodePayeeVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << vnodePayments.mapVnodePayeeVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_VOTE) {
                    if (budget.mapSeenVnodeBudgetVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenVnodeBudgetVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_PROPOSAL) {
                    if (budget.mapSeenVnodeBudgetProposals.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenVnodeBudgetProposals[inv.hash];
                ...
                if (!pushed && inv.type == MSG_VNODE_ANNOUNCE) {
                    if (mnodeman.mapSeenVnodeBroadcast.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenVnodeBroadcast[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Ventual: We use certain sporks during IBD, so check to see if they are
        ...
        if (strCommand == "tx") {
            ...
            //these allow vnodes to publish a limited amount of free transactions
            vRecv >> tx >> vin >> vchSig >> sigTime;

            CVnode* pmn = mnodeman.Find(vin);
            if (pmn != NULL) {
                if (!pmn->allowFreeTx) {
                    //multiple peers can send us a valid vnode transaction
                    if (fDebug) LogPrintf("dstx: Vnode sending too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
                ...
                if (!obfuScationSigner.VerifyMessage(pmn->pubKeyVnode, vchSig, strMessage, errorMessage)) {
                    LogPrintf("dstx: Got bad vnode address signature %s \n", vin.ToString());
                    //pfrom->Misbehaving(20);
                    return false;
                }

                LogPrintf("dstx: Got Vnode transaction %s\n", tx.GetHash().ToString());
    ...
    // getaddr message mitigates the attack.
        ...
        vnodePayments.ProcessMessageVnodePayments(pfrom, strCommand, vRecv);
