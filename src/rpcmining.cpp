...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or ventual.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "        ,...\n"
            "  ],\n"
            "  \"vnode_payments\" : true|false,         (boolean) true, if vnode payments are enabled\n"
            "  \"enforce_vnode_payments\" : true|false  (boolean) true, if vnode payments are enforced\n"
            "}\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Ventual is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Ventual is downloading blocks...");
...
//    result.push_back(Pair("sizelimit", (int64_t)MAX_BLOCK_SIZE));
    ...
    result.push_back(Pair("vnode_payments", pblock->nTime > Params().StartVnodePayments()));
    result.push_back(Pair("enforce_vnode_payments", true));
