...
#include "config/ventual-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ventual.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ventuald.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the VENTUAL and zVNTL money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 7702, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in VENTUAL/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in VENTUAL/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, ventual, (obfuscation, swiftx, vnode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in VENTUAL/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Ventual specific functionality (Vnodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageGroup(_("Vnode options:"));
    strUsage += HelpMessageOpt("-vnode=<n>", strprintf(_("Enable the client to act as a vnode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify vnode configuration file (default: %s)"), "vnode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock vnodes from vnode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-vnodeprivkey=<n>", _("Set the vnode private key"));
    strUsage += HelpMessageOpt("-vnodeaddr=<n>", strprintf(_("Set external address:port to get to this vnode (example: %s)"), "128.127.106.235:7702"));
    ...
    strUsage += HelpMessageOpt("-backupzventual=<n>", strprintf(_("Enable automatic wallet backups triggered after each zVNTL minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeventualamount=<n>     " + strprintf(_("Keep N VENTUAL anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Ventual Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Ventual is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Ventual Core is shutting down."));
...
#endif
    // Make sure only a single Ventual process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Ventual Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Ventual: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZVENTUALMinted();
                        RecalculateZVENTUALSpent();
                    }
                    RecalculateVENTUALSupply(1);
                }
                ...
                // Ventual: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Ventual Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Ventual Core to complete") << "\n";
        ...
        bool fEnableZVentualBackups = GetBoolArg("-backupzventual", true);
        pwalletMain->setZVentualAutoBackups(fEnableZVentualBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
    ...
    CVnodeDB mndb;
    CVnodeDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CVnodeDB::FileError)
        LogPrintf("Missing vnode cache file - mncache.dat, will try to recreate\n");
    ...
    CVnodePaymentDB mnpayments;
    CVnodePaymentDB::ReadResult readResult3 = mnpayments.Read(vnodePayments);

    if (readResult3 == CVnodePaymentDB::FileError)
        LogPrintf("Missing vnode payment cache - mnpayments.dat, will try to recreate\n");
    else if (readResult3 != CVnodePaymentDB::Ok) {
        LogPrintf("Error reading mnpayments.dat: ");
        if (readResult3 == CVnodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }
    ...
    if ((fVNode || vnodeConfig.getCount() > -1) && fTxIndex == false) {
    ...
    if (fVNode) {
        ...
        strVNodeAddr = GetArg("-vnodeaddr", "");

        LogPrintf(" addr %s\n", strVNodeAddr.c_str());

        if (!strVNodeAddr.empty()) {
            CService addrTest = CService(strVNodeAddr);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -vnodeaddr address: " + strVNodeAddr);
            }
        }

        strVNodePrivKey = GetArg("-vnodeprivkey", "");
        if (!strVNodePrivKey.empty()) {
            ...
            if (!obfuScationSigner.SetKey(strVNodePrivKey, errorMessage, key, pubkey)) {
                return InitError(_("Invalid vnodeprivkey. Please see documenation."));
            }

            activeVnode.pubKeyVnode = pubkey;

        } else {
            return InitError(_("You must specify a vnodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    //get the mode of budget voting for this vnode
...
//    }
//
//    nAnonymizeVentualAmount = GetArg("-anonymizeventualamount", 0);
//    if (nAnonymizeVentualAmount > 999999) nAnonymizeVentualAmount = 999999;
//    if (nAnonymizeVentualAmount < 2) nAnonymizeVentualAmount = 2;

    fEnableSwiftTX = GetBoolArg("-enableswifttx", fEnableSwiftTX);
    nSwiftTXDepth = GetArg("-swifttxdepth", nSwiftTXDepth);
    nSwiftTXDepth = std::min(std::max(nSwiftTXDepth, 0), 60);

    //lite mode disables all Vnode and Obfuscation related functionality
    ...
    LogPrintf("Anonymize Ventual Amount %d\n", nAnonymizeVentualAmount);
    ...
    /* Denominations
       ...
       For example:
       1VENTUAL+1000 == (.1VENTUAL+100)*10
       10VENTUAL+10000 == (1VENTUAL+1000)*10
    */
