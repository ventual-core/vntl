...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 VENTUAL at the same time
    ONLY_10000 = 5,                        // find vnode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zVNTL send
enum ZerocoinSpendStatus {
    ZVENTUAL_SPEND_OKAY = 0,                            // No error
    ZVENTUAL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZVENTUAL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZVENTUAL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZVENTUAL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZVENTUAL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZVENTUAL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZVENTUAL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZVENTUAL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZVENTUAL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZVENTUAL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZVENTUAL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZVENTUAL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZVENTUAL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZVENTUAL_SPENT_USED_ZVENTUAL = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZVentualAutoBackups(bool fEnabled)
    {
    ...
    {
        return fMultiSendVnodeReward || fMultiSendStake;
    }
    ...
    /// Get 1000DASH output and keys which can be used for the Vnode
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fVNode && vout[i].nValue == 10000 * COIN) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            // Add vnode collaterals which are handled likc locked coins
            if (fVNode && vout[i].nValue == 10000 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
