...
#define VNODE_ACCEPTED 1
#define VNODE_REJECTED 0
#define VNODE_RESET -1
...
extern std::string strVNodePrivKey;
...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 VENTUAL - checking if valid vnode)
...
private:
    ...
    std::vector<CObfuScationEntry> entries; // Vnode/clients entries
    ...
    bool sessionFoundVnode; //If we've found a compatible Vnode
...
public:
    ...
    {
        if (fVNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            // LogPrint("obfuscation", "CObfuscationPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Vnode. \n");
            return;
        }
    ...
    /// As a client, send a transaction to a Vnode to start the denomination process
    void SendObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    /// Get Vnode updates about the progress of Obfuscation
    ...
    /// Get the denominations for a specific amount of ventual.
