...
CNode* ConnectNode(CAddress addrConnect, const char* pszDest, bool obfuScationMaster)
{
    if (pszDest == NULL) {
        ...
        // so should be safe to skip this and connect to local Hot MN on CActiveVnode::ManageStatus()
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Ventual users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Ventual " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Ventual Core is probably already running."), addrBind.ToString());
        else
...
void RelayInv(CInv& inv)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes){
    		if((pnode->nServices==NODE_BLOOM_WITHOUT_MN) && inv.IsVNodeType())continue;
