...
bool CWallet::GetVnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl* coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseIX) const
{
    ...
    MNCollateral = 1000 * COIN;
    int MNActive = mnodeman.CountEnabled();
    if (MNActive < 25) {
    MNCollateral = 1000 * COIN;
    } else if (MNActive < 50) {
    MNCollateral = 1250 * COIN;
    } else if (MNActive < 75) {
    MNCollateral = 1500 * COIN;
    } else if (MNActive < 100) {
    MNCollateral = 1750 * COIN;
    } else if (MNActive < 125) {
    MNCollateral = 2000 * COIN;
    } else if (MNActive < 150) {
    MNCollateral = 2250 * COIN;
    } else if (MNActive < 200) {
    MNCollateral = 2500 * COIN;
    } else if (MNActive < 250) {
    MNCollateral = 2750 * COIN;
    } else {
    MNCollateral = 3000 * COIN;
    }
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fVNode && (pcoin->vout[i].nValue == 1000 * COIN ||
                                              pcoin->vout[i].nValue == 1250 * COIN ||
                                              pcoin->vout[i].nValue == 1500 * COIN ||
                                              pcoin->vout[i].nValue == 1750 * COIN ||
                                              pcoin->vout[i].nValue == 2000 * COIN ||
                                              pcoin->vout[i].nValue == 2250 * COIN ||
                                              pcoin->vout[i].nValue == 2500 * COIN ||
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fVNode) found = !(pcoin->vout[i].nValue == 1000 * COIN ||
                                                        pcoin->vout[i].nValue == 1250 * COIN ||
                                                        pcoin->vout[i].nValue == 1500 * COIN ||
                                                        pcoin->vout[i].nValue == 1750 * COIN ||
                                                        pcoin->vout[i].nValue == 2000 * COIN ||
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 VENTUAL
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 VENTUAL over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 VENTUAL+1 ( bit on if present )
            // bit 1 - 1000 VENTUAL+1
            // bit 2 - 100 VENTUAL+1
            // bit 3 - 10 VENTUAL+1
            // bit 4 - 1 VENTUAL+1
            // bit 5 - .1 VENTUAL+1
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (fVNode && out.tx->vout[out.i].nValue == MNCollateral) continue; //vnode input
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 VENTUAL.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 VENTUAL.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-ventual-address
                    CScript scriptChange;
...
void CWallet::AutoZeromint()
{
    ...
    // Wait until blockchain + vnodes are fully synced and wallet is unlocked.
    if (!vnodeSync.IsSynced() || IsLocked()){
    ...
    CAmount nBalance = GetUnlockedCoins(); // We only consider unlocked coins, this also excludes vnode-vins
                                           // from being accidentally minted
    CAmount nMintAmount = 0;
    CAmount nToMintAmount = 0;

    // zVNTL are integers > 0, so we can't mint 10% of 9 VENTUAL
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zVNTL\n", nBalance);
        return;
    }

    // Percentage of zVNTL we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zVNTL (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zVNTL amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zVNTL amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zVNTL We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough VENTUAL to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zVNTL. Current percentage of zVNTL: %lf%%\n",
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        ...
        //find vnode rewards that need to be combined
...
bool CWallet::MultiSend()
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        bool sendMSonMNReward = fMultiSendVnodeReward && outpoint.IsVnodeReward(out.tx);
        ...
        if (!walletdb.WriteMSettings(fMultiSendStake, fMultiSendVnodeReward, nLastMultiSendHeight))
        ...
        if (mnSent > 0 && stakeSent > 0)
            return true;
        if (stakeSent > 0 && !fMultiSendVnodeReward)
            return true;
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to VENTUAL
    ...
    // Sign if these are ventual outputs - NOTE that zVNTL outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt)
{
    ...
    receipt.SetStatus("Transaction Mint Started", ZVENTUAL_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus("the selected mint coin is an invalid coin", ZVENTUAL_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason)) {
        receipt.SetStatus("Try to spend with a higher security level to include more coins", ZVENTUAL_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus("the new spend coin transaction did not verify", ZVENTUAL_INVALID_WITNESS);
            return false;
        }
        ...
        //Add the coin spend into a Ventual transaction
        ...
        try {
            ...
            receipt.SetStatus("failed to deserialize", ZVENTUAL_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus("the transaction did not verify", ZVENTUAL_BAD_SERIALIZATION);
            return false;
        }
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                //Tried to spend an already spent zVNTL
                ...
                receipt.SetStatus("the coin spend has been used", ZVENTUAL_SPENT_USED_ZVENTUAL);
                return false;
            }
        }
        ...
        receipt.SetStatus("CoinSpend: Accumulator witness does not verify", ZVENTUAL_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus("Spend Valid", ZVENTUAL_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZVENTUAL_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus("Value is below the the smallest available denomination (= 1) of zVNTL", nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZVENTUAL_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zVNTL transaction
    if (vSelectedMints.empty()) {
        ...
        // Select the zVNTL mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to ventual address to the transaction (the actual primary spend taking place)
    ...
    receipt.SetStatus("Transaction Created", ZVENTUAL_SPEND_OKAY); // Everything okay
...
void CWallet::ZVentualBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozventualbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozventualbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozventualbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZVENTUAL_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZVENTUAL_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZVENTUAL_COMMIT_FAILED;
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZVENTUAL_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& mint : vNewMints) {
            if (!walletdb.EraseZerocoinMint(mint)) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZVENTUAL_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    receipt.SetStatus("Spend Successful", ZVENTUAL_SPEND_OKAY);  // When we reach this point spending zVNTL was successful
