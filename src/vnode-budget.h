...
#ifndef VNODE_BUDGET_H
#define VNODE_BUDGET_H
...
//
// CBudgetVote - Allow a vnode node to vote and broadcast throughout the network
//
...
//
// CFinalizedBudgetVote - Allow a vnode node to vote and broadcast throughout the network
//
...
public:
    ...
    std::map<uint256, CBudgetProposalBroadcast> mapSeenVnodeBudgetProposals;
    std::map<uint256, CBudgetVote> mapSeenVnodeBudgetVotes;
    std::map<uint256, CBudgetVote> mapOrphanVnodeBudgetVotes;
    ...
    void ClearSeen()
    {
        mapSeenVnodeBudgetProposals.clear();
        mapSeenVnodeBudgetVotes.clear();
    ...
    void Clear()
    {
        ...
        mapSeenFinalizedBudgets.clear();
        mapSeenFinalizedBudgetVotes.clear();
        mapOrphanVnodeBudgetVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        READWRITE(mapSeenVnodeBudgetProposals);
        READWRITE(mapSeenVnodeBudgetVotes);
        READWRITE(mapSeenFinalizedBudgets);
        READWRITE(mapSeenFinalizedBudgetVotes);
        READWRITE(mapOrphanVnodeBudgetVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (vnode only)
...
public:
    ...
    void AutoCheck();
    //total ventual paid out by this budget
    CAmount GetTotalPayout();
    //vote on this finalized budget as a vnode
    void SubmitVote();
...
//
// Budget Proposal : Contains the vnode votes for each budget
//
