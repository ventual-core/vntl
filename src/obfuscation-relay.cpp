...
CObfuScationRelay::CObfuScationRelay(CTxIn& vinVnodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinVnode = vinVnodeIn;
    vchSig = vchSigIn;
...
std::string CObfuScationRelay::ToString()
{
    ...
    info << "vin: " << vinVnode.ToString() << " nBlockHeight: " << (int)nBlockHeight << " nRelayType: " << (int)nRelayType << " in " << in.ToString() << " out " << out.ToString();
...
{
    CVnode* pmn = mnodeman.GetVnodeByRank(nRank, nBlockHeight, ActiveProtocol());
