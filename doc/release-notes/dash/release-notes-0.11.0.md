...
- Added new DNS seed from vnode.io
...
- Improve support for start-many with multi vnode config
- New vnode rpc commands: stop-many, start-alias, stop-alias, list-conf
- Fixed possible vnode payments exploit
- Better support for non-specific vnode payments
- Added vnode support for regtest
- Randomly sort vnodes before picking next winner
- Show number of vnodes in debug window
...
- Add '-disablewallet' mode to let ventuald run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with ventuald
