...
- saction for Darksend. These are special transactions that require a signature that only the vnodes can create.
...
- Clients remember which vnodes they've connected to in the past and won't use them against.
...
- Split up darksend.cpp into vnode.cpp, activevnode.cpp and instantx.cpp
- Added modular ProcessMessages for Darksend, Vnodes and InstantX
- Client can now join sessions with any other users

0.10.15.20/21 Release notes
---------------------------

- added bloom filters for dsee/dseep broadcasts, moved expensive dsee search, vnode vote caching
...
- Fixed vnode payment check for out-of-order blocks
...
- vnode only take newest dsee now


0.10.15.15 Release notes
------------------------

- fixed fragmentation issue, allow vnodes to update pubkey2
...
- Fixed the stuck blockchain (due to vnode payments)
- Improves collateral creation for those getting "invalid collateral"
- Minimum confirmation requirements before vnode payment.
  Must have more confirmations then there are vnodes.
...
- Changes the vnode payments keys, you'll only recieve vnode
...
- I've made some huge progress with enforcement. Each vnode will
...
- Fixed some issues with vnode payments syncing
...
- Fixed some issues with the way the vnode payment list
  was being handled
- Vnode confirmation error now displays 200 required instead of 6
...
- security update: This update simply verifies that the signature was
  not forged and that the vnodes entering the list are authentic.
...
- vnode constants for communication are much easier to read now
...
- Improved handling of enforcement for bad vnode lists
- Removed old vnode override code
...
- Darksend UI is disabled for vnodes now and titlebar says "[vnode]"
- Improvement for dealing with splitting large inputs
- Protocol version bump to kick old vnodes off
...
- Added a spork for enforcing vnode payments (this will ensure misconfigured
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  I've modified the queuing system, so users wait in a vnode queue without
...
- removed matching vnode debugging messages, that's not really an error
- Client now prioritises sending denominated funds back into Darksend. This will
  ...
  configuration.
- fixed a bug where vnodes send failed transactions
...
- Darksend now checks diskspace before sending anything to a vnode.
...
- Disabled collateral charging for now. We'll work on this after RC4 is
  released and update the vnode network after it's working properly. It's
...
- I've switched up the way the vnode network works.
    1.) Users now will join a random vnode (1 of the entire list, just
        completely randomly)
    2.) Upon joining if it's the first user, the vnode will propagate
...
- Fixed some bugs with connecting to the correct vnodes
...
- Added session IDs for vnode communication. Clients were getting
  confused when they got messages about other sessions (sometimes happened when
  they all jumped on the same vnode at once)
- Added a pre-session state where the client will query a random vnode
...
- Added code to randomly use the top 20 vnodes, this can dynamically be
...
- After successful transactions clients will now automatically attempt another
  session on a random vnode, then repeat until they get any kind of error
  or run out of funds that need to be processed.
- Fixed a change address reuse issue
- Fixed an issue with the compatible join algorithm (Vnodes will only join
...
- Fixed an issue where clients weren't connected to the correct vnode
- Fixed vnode relay issues
...
- This version automatically resets the vnode state machine after a short
  period of inactivity.
- Updated protocol version to kick old vnodes off
...
- Found a communication mix up where clients would see messages from the wrong
  vnode and think it was theirs, also causing collateral fees
...
- incremented protocol version to force vnode updates
...
- Client will only submit 1 transaction into the pool fixing possible
  anonymity issues
- Vnodes will only merge compatible transactions using the same
...
- Added possible fix for vnode list syncing
...
- Vnodes now reset themselves when they give "entries is full".
...
on the same vnode for testing purposes:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usevnode="192.168.56.102:51474"
/darkcoin-qt -datadir=/home/user/.darkcoin2 -listen=0 -darksendrounds=8 -usevnode="192.168.56.102:51474"

and even disable darksend auto-denom if wanted:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usevnode="192.168.56.102:51474" -disabledarksend=1
...
- Vnodes should change every block now (missed a mod=10 last time)
...
- Vnodes should change every block now
...
- usevnode option, will override active vnode (only in RC, just for testing)
...
- Fixed payout issues (vnode consessus was paying out to vout(0) by default)
- Improved DarksendInput add entry verification. Vnodes will now reject
...
- Fixed a signing bug with the vnode voting system causing a bunch of issues
- Updated unit tests
