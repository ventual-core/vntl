...
Ventual now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
* finalized budget voting - vnodes that agree with the finalization will vote on that budget
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ventual_address\> \<monthly_payment_ventual\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ Ventual, _12_ times over the course of a year totaling _24000_ Ventual.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ventual_address\> \<monthly_payment_ventual\> \<collateral_hash\>
...
If everything looks correct, you can ask for votes from other vnodes. To vote on a proposal, load a wallet with _vnode.conf_ file. You do not need to access your cold wallet to vote for proposals.
...
After you get enough votes, execute `mnbudget projection` to see if you made it into the budget. If you the budget was finalized at this moment which proposals would be in it. Note: Proposals must be active at least 1 day on the network and receive 10% of the vnode network in yes votes in order to qualify (E.g. if there is 2500 vnodes, you will need 250 yes votes.)
...
When block `1000000` is reached you'll receive a payment for `1200` Ventual.
